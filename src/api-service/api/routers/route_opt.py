...
async def post_coord_from_chat(chat_id: str, x_session_id: str = Header(None, alias="X-Session-ID")):
    ...
    iti_first_draft['messages'].append({"ordered_locations": ordered_locations, "ordered_coordinates": ordered_coordinates})
    ...
    return {"ordered_locations": ordered_locations, "ordered_coordinates": ordered_coordinates}

@router.get("/loc_coord/{chat_id}")
async def get_coord_from_chat(chat_id: str, x_session_id: str = Header(None, alias="X-Session-ID")):
    # Construct the file path
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Index out of range when accessing 'messages': {str(e)}")
    except Exception as e:
