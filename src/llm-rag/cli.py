...
def generate_query_embedding(query):
    ...
    kwargs = (dict(output_dimensionality=EMBEDDING_DIMENSION) 
              if EMBEDDING_DIMENSION else {})
    embeddings = (embedding_model
                  .get_embeddings(query_embedding_inputs, **kwargs))
    return embeddings[0].values


def generate_text_embeddings(chunks, 
                             dimensionality: int = 256, batch_size=250):
...
def chunk(method="char-split"):
    ...
    for text_file in text_files:
        ...
        if text_chunks is not None:
            ...
            jsonl_filename = (os.path.
                              join(OUTPUT_FOLDER, 
                                   f"chunks-{method}-{city_name}.jsonl"))
...
def embed(method="char-split"):
    ...
    jsonl_files = (glob.glob(os.path
                             .join(OUTPUT_FOLDER, 
                                   f"chunks-{method}-*.jsonl")))
    ...
    for jsonl_file in jsonl_files:
        ...
        if method == "semantic-split":
            embeddings = (generate_text_embeddings(chunks, 
                          EMBEDDING_DIMENSION, batch_size=10))
        else:
            embeddings = (generate_text_embeddings(chunks, 
